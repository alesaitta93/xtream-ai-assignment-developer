...
CHALLENGE_DIR = os.path.abspath(os.path.join(os.getcwd(), os.pardir))
OUT_DIR = os.path.join(*[CHALLENGE_DIR, "pipeline", "models"])
MODEL_INDEX_FILENAME = "models.csv"
DUMMY_COLS_FILENAME = "dummyfied_data_columns.json"

# Model independent utilities

def data_preparation(df, test_size=0.2, random_state=42, out_dir=OUT_DIR, dummy_cols_filename=DUMMY_COLS_FILENAME):
    ...
    # Saving index of data encoded df
    cols = diamonds_dummy.columns.to_list()
    dummy_cols_path = os.path.join(out_dir, dummy_cols_filename)
    if not os.path.isdir(out_dir):
        os.mkdir(out_dir)
    if not os.path.isfile(dummy_cols_path):
        with open(dummy_cols_path, "w") as out_file:
            json.dump({"dummy_columns": cols}, out_file)
...
def save_model(model, r2, mae, dataset_path, csv_model_index_filename=MODEL_INDEX_FILENAME, out_dir=OUT_DIR):
...
def train_n_evaluate_linear_model(df):
...
def train_n_save_linear_model(path):
    df = pd.read_csv(path)
    model, r2, mae = train_n_evaluate_linear_model(df)
    save_model(model, r2, mae, path)

# xgboost utilities

# def data_preparation_for_xgb(df, test_size=0.2, random_state=42):
#     diamonds = df
...
#     else:
#         x_train_xbg = diamonds_processed_xgb.drop(columns='price')
#         x_test_xbg = None
#         y_train_xbg = diamonds_processed_xgb['price']
#         y_test_xbg = None
...
def optimize_train_n_evaluate_xgboost(path):
    df = pd.read_csv(path)
    # x_train_xbg, x_test_xbg, y_train_xbg, y_test_xbg = data_preparation_for_xgb(df)
    x_train_xbg, x_test_xbg, y_train_xbg, y_test_xbg = data_preparation(df)
    ...
    model = xgboost.XGBRegressor(**params, random_state=42)
